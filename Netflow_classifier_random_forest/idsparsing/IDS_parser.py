import logging
from idsparsing.common import parse_filebeat_msg
from idsparsing.kafka_utils import *
from pprint import pprint
from idsparsing.snort2 import parse_snort2_alert_fast_log_line, parsed_snort2_alert_to_TCAM_event
from idsparsing.suricata import parse_suricata_log_line, parsed_suricata_alert_to_TCAM_event
from idsparsing.wazuh import parse_wazuh_log_line, parsed_wazuh_alert_to_TCAM_event
import os
import time
...
def proc_msg(topic, value, producer,msg_out_cnt):
    global alertCache
    ...
    if topic in [KAFKA_TOPIC_IN_SNORT2, KAFKA_TOPIC_IN_SURICATA]:
        ...
        return
    clearCache()
    if ids_name=="Snort2" or ids_name=="Suricata":
        cacheTuple=(TCAM_event["Threat_Label"],TCAM_event["Threat_Category"],TCAM_event["Source_Address"],TCAM_event["Destination_Address"])
    if ids_name=="Wazuh":
        cacheTuple=(TCAM_event["wazuh_agent_id"],TCAM_event['Threat_Label'],TCAM_event['Threat_Category'],TCAM_event['Source_IP'])
    print(cacheTuple)
    cacheKeys=alertCache.keys()
    if cacheTuple in cacheKeys:
        return
    #import os
    #import requests
    TENANT_ID = 20094#os.environ['TENANT_ID']
    #PARTITION=requests.get('http://tenant-api-service.ti-dcp:6000/api/partition/'+str(TENANT_ID)).json()['partition']
    TCAM_event["tenantId"]=TENANT_ID
    producer.send(KAFKA_TOPIC_OUT, key='IDS-parser-msg-%d' % msg_out_cnt, value=TCAM_event)
    if ids_name=="Snort2" or ids_name=="Suricata":
        alertCache[cacheTuple]=time.time()
    if ids_name=="Wazuh":
        alertCache[cacheTuple]=time.time()
    ...
    try:
        for message in consumer:
            logger.info("%s:%d:%d key=%s value=%s" % (message.topic, message.partition,
                                          ...
                                          message.value))
            proc_msg(message.topic, message.value, producer,msg_cnt)
            msg_cnt += 1
