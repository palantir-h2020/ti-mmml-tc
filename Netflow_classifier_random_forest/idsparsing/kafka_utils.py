        ...
        try:
            ...
            if topic is not None:
                consumer = KafkaConsumer(
                     ...
                     value_deserializer=safe_deserializer_csv if value_deserializer == 'csv' else safe_deserializer_json,partioner=partioner)
            else:
                consumer = KafkaConsumer(
                     bootstrap_servers=bootstrap_servers,
                     group_id=group_id,
    ...
    while not is_connected:
        try:
            ...
            producer = KafkaProducer(
                 ...
                 value_serializer=safe_serializer_csv if value_serializer == 'csv' else safe_serializer_json)#TODO: add partioner method
            is_connected = True
        except NoBrokersAvailable:
            attempt_num += 1
            if attempt_num < max_connect_attempts:
